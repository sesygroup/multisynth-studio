...
<script type="text/javascript" charset="utf-8">
      ...
      var options = {
         ...
         manipulation: {
        	addNode: function(data, callback) {
               ...
               $('#bp-node-popUp-modal-label').text("<spring:message code='bp.state.create' />");
            ...
            editNode: function(data, callback) {
               ...
               $('#bp-node-popUp-modal-label').text("<spring:message code='bp.state.update' />");
            ...
            addEdge: function(data, callback) {
               $('#bp-transition-action').val('create');
               ...
               $('#bp-transition-popUp-modal-label').text("<spring:message code='bp.transition.create' />");
               $("#saveTransitionButton").text("<spring:message code='common.create' />");
            ...
            editEdge: function(data, callback) {
               $('#bp-transition-action').val('update');
               $('#bp-transition-id').val(data.id);
               $('#bp-transition-node-from-id').val(data.from);
               $('#bp-transition-node-from').val(bp_nodesDataset.get(data.from).name);
               $('#bp-transition-node-to-id').val(data.to);
               $('#bp-transition-node-to').val(bp_nodesDataset.get(data.to).name);
               $('#bp-transition-type').val(bp_edgesDataset.get(data.id).type).change();
               $('#bp-transition-typedmessage-name').val(bp_edgesDataset.get(data.id).typedMessageName);
               $("#bp-transition-typedmessage-name").parsley().reset();
               $('#bp-transition-typedmessage-content').val(bp_edgesDataset.get(data.id).typedMessageContent);
               $("#bp-transition-typedmessage-content").parsley().reset();
               $('#bp-transition-popUp-modal-label').text("<spring:message code='bp.transition.update' />");
               $("#saveTransitionButton").text("<spring:message code='common.update' />")
               $('#bp-transition-popUp-modal').modal('show');
               callback(null);
            }
         }
   ...
   function showBpEdgeProperty(edgeID) {
      $('#property-bp-transition-typedmessage-name').parsley().reset();
      $('#property-bp-transition-typedmessage-content').parsley().reset();
   ...
   function saveNode() {
      ...
      if ($("#bp-state-name").parsley().isValid() === true) {
         if ($('#bp-state-action').val() === 'create') {
            ...
            // update all titles of all transitions connected with whis state
            bp_network.getConnectedEdges($('#bp-state-id').val()).forEach(function(transitionID) {
               var transitionObj = bp_edgesDataset.get(transitionID);
               var typedMessage = '(' + transitionObj.typedMessageName + ',' + transitionObj.typedMessageContent + ')';
               if (transitionObj.type === 'sendAction') {
                  typedMessage = typedMessage + '!'
               } else if (transitionObj.type === 'receiveAction') {
                  typedMessage = typedMessage + '?'
               } else if (transitionObj.type === 'internalAction') {
                  typedMessage = 'epsilon'
               }
               bp_edgesDataset.update({
                  id: transitionID,
                  title: "(" + bp_nodesDataset.get(transitionObj.from).name + ", " + typedMessage + ", " + bp_nodesDataset.get(transitionObj.to).name + ")"
               });
            });
            $('#bp-state-id').val('');
      ...
      if ($('#bp-transition-type').val() === 'sendAction' || $('#bp-transition-type').val() === 'receiveAction') {
         ...
         if ($("#bp-transition-typedmessage-name").parsley().isValid() === true && $("#bp-transition-typedmessage-name").parsley().isValid() === true) {
            if ($('#bp-transition-type').val() === 'sendAction') {
               label = $("#bp-transition-typedmessage-name").val() + "!";
               labelComplete = "(" + $("#bp-transition-typedmessage-name").val() + "," + $("#bp-transition-typedmessage-content").val() + ")!";
            } else if ($('#bp-transition-type').val() === 'receiveAction') {
               label = $("#bp-transition-typedmessage-name").val() + "?";
               labelComplete = "(" + $("#bp-transition-typedmessage-name").val() + "," + $("#bp-transition-typedmessage-content").val() + ")?";
            }
            if ($('#bp-transition-action').val() === 'create') {
               bp_edgesDataset.add({
                  from: $("#bp-transition-node-from-id").val(),
                  to: $("#bp-transition-node-to-id").val(),
                  label: label,
                  title: "(" + $("#bp-transition-node-from").val() + ", " + labelComplete + ", " + $("#bp-transition-node-to").val() + ")",
               ...
               $("#bp-transition-id").val('');
               showMessage("success", "<spring:message code='common.success' />", "<spring:message code='bp.transition.update.success.message' />");
            }
   ...
   $(document).ready(function() {
      ...
      // trigger save botton when we press enter on the message name or on the message content of the transition
      $("#bp-transition-typedmessage-name").keypress(function(event) {
         if (event.which == 13) {
            event.preventDefault();
            $("#saveTransitionButton").click();
         }
      });
      $("#bp-transition-typedmessage-content").keypress(function(event) {
         if (event.which == 13) {
            event.preventDefault();
            $("#saveTransitionButton").click();
         }
      });
      $("#property-bp-transition-typedmessage-name").keypress(function(event) {
         if (event.which == 13) {
            event.preventDefault();
            $("#property-bp-transition-update").click();
         }
      });
      $("#property-bp-transition-typedmessage-content").keypress(function(event) {
         if (event.which == 13) {
            event.preventDefault();
            $("#property-bp-transition-update").click();
         }
      ...
      $("#property-bp-state-update").click(function(e) {
         ...
         if ($('#property-bp-state-name').parsley().isValid() === true) {
            ...
            // update all titles of all transitions connected with whis state
            bp_network.getConnectedEdges($('#property-bp-state-id').val()).forEach(function(transitionID) {
               var transitionObj = bp_edgesDataset.get(transitionID);
               var typedMessage = '(' + transitionObj.typedMessageName + ',' + transitionObj.typedMessageContent + ')';
               if (transitionObj.type === 'sendAction') {
                  typedMessage = typedMessage + '!'
      ...
      // update transition
      $("#property-bp-transition-update").click(function(e) {
         e.preventDefault();
         var label = "";
         var labelComplete = "";
         if ($('#property-bp-transition-type').val() === 'sendAction' || $('#property-bp-transition-type').val() === 'receiveAction') {
            $("#property-bp-transition-typedmessage-name").parsley().validate();
            $("#property-bp-transition-typedmessage-content").parsley().validate();
            if ($("#property-bp-transition-typedmessage-name").parsley().isValid() === true && $("#property-bp-transition-typedmessage-name").parsley().isValid() === true) {
               if ($('#property-bp-transition-type').val() === 'sendAction') {
                  label = $("#property-bp-transition-typedmessage-name").val() + "!";
                  labelComplete = "(" + $("#property-bp-transition-typedmessage-name").val() + "," + $("#property-bp-transition-typedmessage-content").val() + ")!";
               } else if ($('#property-bp-transition-type').val() === 'receiveAction') {
                  label = $("#property-bp-transition-typedmessage-name").val() + "?";
                  labelComplete = "(" + $("#property-bp-transition-typedmessage-name").val() + "," + $("#property-bp-transition-typedmessage-content").val() + ")?";
               }
               bp_edgesDataset.update({
                  id: $('#property-bp-transition-id').val(),
                  label: label,
                  title: "(" + $("#property-bp-transition-from").val() + ", " + labelComplete + ", " + $("#property-bp-transition-to").val() + ")",
                  type: $("#property-bp-transition-type").val(),
                  typedMessageName: $("#property-bp-transition-typedmessage-name").val(),
                  typedMessageContent: $("#property-bp-transition-typedmessage-content").val(),
                  dashes: false
               });
               $('#property-bp-transition-label').val(label);
               showMessage("success", "<spring:message code='common.success' />", "<spring:message code='bp.transition.update.success.message' />");
            }
         } else {
            label = "epsilon"
            bp_edgesDataset.update({
               id: $('#property-bp-transition-id').val(),
               label: label,
               title: "(" + $("#property-bp-transition-from").val() + ", " + label + ", " + $("#property-bp-transition-to").val() + ")",
               type: $("#property-bp-transition-type").val(),
               typedMessageName: "",
               typedMessageContent: "",
               dashes: false
      ...
      $("#testMoonService").click(function(e) {
         e.preventDefault();
