...
<script type="text/javascript" charset="utf-8">
      ...
      var options = {
         ...
         manipulation: {
            addNode: function(data, callback) {
               var participantNames = cpb_getParticipantNames();
   ...
   function cpb_participantExists(participantName) {
      var exists = false;
      cpb_nodesDataset.get().forEach(function(node) {
   ...
   function cpb_transitionExists(sourceNode, targetNode, type, message) {
      var exists = false;
   ...
   function cpb_stateExists(stateName, participantName) {
      var exists = false;
      cpb_nodesDataset.get().forEach(function(node) {
   ...
   function cpb_getParticipantNames() {
   ...
   function saveCpbTransition() {
      ...
      if ($("#cpb-transition-message").parsley().isValid() === true) {
         if (cpb_transitionExists($("#cpb-transition-node-from-id").val(), $("#cpb-transition-node-to-id").val(), $("#cpb-transition-type").val(), transitionMessage)) {
   ...
   $(document).ready(function() {
      ...
      window.Parsley.addValidator('participant_selection_exists', {
         validateString: function(value) {
            return !cpb_participantExists(value);
         }
      });

      window.Parsley.addValidator('state_selection_exists', {
         validateString: function(value) {
            return !cpb_stateExists(value, $("#cpb-state-participant").val());
         }
      });
